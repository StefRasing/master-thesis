...
#SBATCH --time=72:00:00         # Request run time (wall-clock). Default is 1 minute
...
#SBATCH --output=exe/out/p_b_%j.out # Set name of output log. %j is the Slurm jobId
#SBATCH --error=exe/out/p_b_%j.err  # Set name of error log. %j is the Slurm jobId

export JULIA_DEPOT_PATH=$TMPDIR/julia_depot
mkdir -p $JULIA_DEPOT_PATH

srun julia --project=master-thesis -e 'include("experiments/transferable_phalcon_bitvectors.jl")' 3