...
#SBATCH --output=exe/out/h_s_%j.out # Set name of output log. %j is the Slurm jobId
#SBATCH --error=exe/out/h_s_%j.err  # Set name of error log. %j is the Slurm jobId

export JULIA_DEPOT_PATH=$TMPDIR/julia_depot
mkdir -p $JULIA_DEPOT_PATH

srun julia --project=master-thesis -e 'include("experiments/heuristic_strings.jl")'