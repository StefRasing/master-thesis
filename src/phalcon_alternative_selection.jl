abstract type AbstractPhalconAlternativeSelection <: ProgramIterator end
...
@programiterator mutable PhalconAlternativeSelection(
    problem = nothing,
    ...
    max_number_of_property_selection_rounds::Int = typemax(Int),

    properties_selected_per_round::Int = 1,
    heuristic_increase_threshold::Float64 = 1.0,
...
) <: AbstractPhalconAlternativeSelection

function heuristic_cost(iter::PhalconAlternativeSelection, program::AbstractRuleNode)::Number
...
"""
function add_to_pool!(iter::PhalconAlternativeSelection, program::AbstractRuleNode, parent=nothing)    
...
"""
function initialize!(iter::PhalconAlternativeSelection)
    # ---------------------------
    #   1. Setup fields
    # ---------------------------
...
function neighborhood(iter::PhalconAlternativeSelection, program::AbstractRuleNode)
    grammar = iter.solver.grammar
    types = grammar.types
...
"""
function combine!(iter::PhalconAlternativeSelection)
...
function refine_heuristic!(iter::PhalconAlternativeSelection)
    ...
    threshold = Int(round(optimal_heuristic_increase * iter.heuristic_increase_threshold))

    best_properties = []

    for (property, target_values) in iter.candidate_properties
        increases = [[target_value .!= interp(property, (io.in[:_arg_out] = output; io.in)) for (output, io, target_value) in zip(pool_entry.program._val, iter.problem.spec, target_values)] for pool_entry in iter.pool]
        increase = sum(vcat(increases...))

        index = searchsortedfirst([e[1] for e in best_properties], increase, rev=true)

        if increases in [e[2] for e in best_properties]
            continue
        end

        entry = (increase, increases, property, target_values)    
        insert!(best_properties, index, entry)

        if length(best_properties) > iter.properties_selected_per_round
            pop!(best_properties)
        end

        if length(best_properties) >= iter.properties_selected_per_round && best_properties[iter.properties_selected_per_round][1] >= threshold
            break
        end
    end

    for (increase, increases, property, target_values) in best_properties
        push!(iter.selected_properties, (property, target_values))
        # println("\nAdded property ($increase / $threshold): $target_values")
        # @show increases
...
"""
function Base.iterate(iter::PhalconAlternativeSelection)
    initialize!(iter)
...
"""
function Base.iterate(iter::PhalconAlternativeSelection, state::Vector{<:AbstractRuleNode})
    ...
    if isempty(state)
        if length(iter.selected_properties) >= iter.max_number_of_property_selection_rounds * iter.properties_selected_per_round
            return nothing
        end
