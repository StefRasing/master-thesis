abstract type AbstractPhalconLimitedIncreaseUniqueProperties <: ProgramIterator end

@programiterator mutable PhalconLimitedIncreaseUniqueProperties(
    problem = nothing,
    ...
    max_increase_percentage::Float64 = 1.0,
...
) <: AbstractPhalconLimitedIncreaseUniqueProperties

function heuristic_cost(iter::PhalconLimitedIncreaseUniqueProperties, program::AbstractRuleNode)::Number
...
"""
function add_to_pool!(iter::PhalconLimitedIncreaseUniqueProperties, program::AbstractRuleNode, parent=nothing)    
...
"""
function initialize!(iter::PhalconLimitedIncreaseUniqueProperties)
    # ---------------------------
    #   1. Setup fields
    # ---------------------------
...
function neighborhood(iter::PhalconLimitedIncreaseUniqueProperties, program::AbstractRuleNode)
    grammar = iter.solver.grammar
    types = grammar.types
...
"""
function combine!(iter::PhalconLimitedIncreaseUniqueProperties)
...
function refine_heuristic!(iter::PhalconLimitedIncreaseUniqueProperties)
    ...
    optimal_heuristic_increase = Int(round(iter.max_increase_percentage * optimal_heuristic_increase))
    ...
    for (property, target_values) in iter.candidate_properties
        if (property, target_values) in iter.selected_properties
            continue
        end
...
"""
function Base.iterate(iter::PhalconLimitedIncreaseUniqueProperties)
    initialize!(iter)
...
"""
function Base.iterate(iter::PhalconLimitedIncreaseUniqueProperties, state::Vector{<:AbstractRuleNode})
    ...
    if isempty(state)
        ...
        for e in iter.extensions
            if iter.solver.grammar.types[get_rule(e)] == iter.solver.grammar.rules[1]
                add_to_pool!(iter, e)
            end
        end
