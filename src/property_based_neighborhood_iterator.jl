...
@programiterator PropertyBasedNeighborhoodIterator(
    ...
    pool_size::Int,
    candidate_properties::Vector{Tuple{AbstractRuleNode, Any}},
    ...
    programs_evaluated::Base.RefValue{Int} = Ref(0),
    property_grammar = grammar,
) <: AbstractPropertyBasedNeighborhoodIterator


function heuristic_cost(iter::PropertyBasedNeighborhoodIterator, program::AbstractRuleNode)::Number
    ...
    return count(
        interp(property, (io.in[:_arg_out] = output; io.in)) != target_value
...
function Base.iterate(iter::PropertyBasedNeighborhoodIterator, state::Vector{<:AbstractRuleNode})
    ...
    if isempty(state)
        ...
        for e in iter.extensions 
            if iter.solver.grammar.types[get_rule(e)] == get_starting_symbol(iter)
                add_to_pool!(iter, e)
            end
        end
