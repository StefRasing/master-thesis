...
function make_plots(files, field, plot_kwargs, series_kwargs)
    ...
    problems = occursin("bitvectors", files[1]) ? 151 : occursin("strings", files[1]) ? 124 : 400
...
function strings_time_vs_acc()
    plot_kwargs = (
        ...
        title = "Execution time for SyGuS strings",
    )
    series_kwargs = [
...
function strings_enum_vs_acc()
    plot_kwargs = (
        ...
        ylims = (0, 124),
        right_margin = 10Plots.mm,
        title = "Program evaluations for SyGuS strings",
    )
    series_kwargs = [
        (label = "Phalcon", color = RGB(0.00, 0.45, 0.70)),
        (label = "Genetic Search (Levenshtein)", color = RGB(0.84, 0.37, 0.00)),
...
function bitvectors_time_vs_acc()
    plot_kwargs = (
        ...
        ylims = (0, 160),
        right_margin = 10Plots.mm,
        title = "Execution time for SyGuS bitvectors",
    )
    series_kwargs = [
        (label = "Phalcon", color = RGB(0.00, 0.45, 0.70)),
        (label = "Genetic Search (edit distance)", color = RGB(0.84, 0.37, 0.00)),
        # (label = "Heuristic Search", color = RGB(0.10, 0.65, 0.15)),
    ]
    names = [
        "phalcon_bitvectors", 
...
function bitvectors_enum_vs_acc()
    plot_kwargs = (
        xticks = (r = 3:7; ([10.0^n for n in r], ["10e$n" for n in r])),
        xscale = :log10,
        yticks = 0:20:160,
        ylims = (0, 160),
        right_margin = 10Plots.mm,
        title = "Program evaluations for SyGuS bitvectors",
    )
    series_kwargs = [
        (label = "Phalcon", color = RGB(0.00, 0.45, 0.70)),
        (label = "Genetic Search (edit distance)", color = RGB(0.84, 0.37, 0.00)),
...
function arc_time_vs_acc()
    plot_kwargs = (
        xticks = (r = 0:4; ([10.0^n for n in r], ["10e$n" for n in r])),
        xscale = :log10,
        yticks = 0:50:400,
        ylims = (0, 410),
        right_margin = 10Plots.mm,
        title = "Execution time for ARC-AGI 1 training tasks",
    )
    series_kwargs = [
        (label = "Phalcon", color = RGB(0.00, 0.45, 0.70)),
        (label = "Genetic Search (edit distance)", color = RGB(0.84, 0.37, 0.00)),
    ...
    files = ["data/$(name).json" for name in names]
    make_plots(files, :execution_time, plot_kwargs, series_kwargs)
    savefig("plots/arc_time_vs_acc.png")
end
...
function arc_enum_vs_acc()
    plot_kwargs = (
        ...
        ylims = (0, 410),
        right_margin = 10Plots.mm,
        title = "Program evaluations for ARC-AGI 1 training tasks",
    )
    series_kwargs = [
        (label = "Phalcon", color = RGB(0.00, 0.45, 0.70)),
        (label = "Genetic Search (edit distance)", color = RGB(0.84, 0.37, 0.00)),
    ...
    files = ["data/$(name).json" for name in names]
    make_plots(files, :programs_enumerated, plot_kwargs, series_kwargs)
    savefig("plots/arc_enum_vs_acc.png")
end
