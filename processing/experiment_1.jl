...
function strings_time_vs_acc_bustle()
    plot_kwargs = (
        ...
        title = "Comparing performance of Phalcon and CrossBeam",
    )
    series_kwargs = [
        (label = "Phalcon", color = RGB(0.00, 0.45, 0.74), ),
        (label = "CrossBeam", color = RGB(0.85, 0.33, 0.10), ),
    ]
    names = [
        "bustle_and_crossbeam/phalcon_strings_bustle", 
        "bustle_and_crossbeam/crossbeam_strings", 
    ]
    files = ["data/$(name).json" for name in names]
    make_plots(files, :execution_time, plot_kwargs, series_kwargs)
    savefig("plots/strings_time_vs_acc_bustle.png")
end
...
function strings_enum_vs_acc_bustle()
    ...
    savefig("plots/strings_enum_vs_acc_bustle.png")
end
...
#=
    ...
    Time vs Accuracy
=#
function strings_time_vs_acc_absynth()
    plot_kwargs = (
        xticks = ([10.0^n for n in -2:3], ["10e$n" for n in -2:3]),
        xscale = :log10,
        yticks = 0:10:90,
        right_margin = 10Plots.mm,
        title = "Comparing performance of Phalcon and Absynth",
    )
    series_kwargs = [
        (label = "Phalcon", color = RGB(0.00, 0.45, 0.74), ),
        (label = "Absynth", color = RGB(0.85, 0.33, 0.10), ),
    ]
    names = [
        "absynth/phalcon_strings_absynth", 
        "absynth/absynth_strings", 
    ]
    files = ["data/$(name).json" for name in names]
    make_plots(files, :execution_time, plot_kwargs, series_kwargs)
    savefig("plots/strings_time_vs_acc_absynth.png")
end
...
#=
    ...
    Enum vs Accuracy
=#
function strings_enum_vs_acc_absynth()
    plot_kwargs = (
        xticks = ([10.0^n for n in 1:6], ["10e$n" for n in 1:6]),
        xscale = :log10,
        yticks = 0:10:90,
    ...
    files = ["data/$(name).json" for name in names]
    make_plots(files, :programs_enumerated, plot_kwargs, series_kwargs)
    savefig("plots/strings_enum_vs_acc_absynth.png")
end

strings_time_vs_acc_bustle()
strings_enum_vs_acc_bustle()
strings_time_vs_acc_absynth()
strings_enum_vs_acc_absynth()