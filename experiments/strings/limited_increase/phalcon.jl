...
include("../../../src/phalcon_limited_increase.jl")
...
function phalcon_synthesizer(;
    iterator::PhalconLimitedIncrease,
    problem::Problem,
)
...
params = n -> merge(default_params, (max_increase_percentage = n,))
path = n -> "data/strings/limited_increase/a=$n.jld2"
# as = [parse(Float64, ARGS[1])]
as = [0.7, 0.8, 0.9, 1.0]

data = [
    @benchmark PhalconLimitedIncrease params=params(n) problems=problems[1:1] grammars=grammars args=arguments path=path(n) 
    for n in as
]

savefig(problems_solved_over_time(data; 
    label = r->"Max increase $(Int(r.params[:max_increase_percentage]*100))%", 
    xscale = :log10,
    xticks = [10.0^n for n in 0:5],
    xlims = [10^0/1.1, 1.1*10^4],
    legend = :topleft,
), "plots/strings/limited_increase/problems_solved_over_time.png")
...
savefig(problems_solved_over_enumerations(data, 
    ...
    legend = :topleft,
), "plots/strings/limited_increase/problems_solved_over_enumerations.png")

savefig(problems_solved_over_number_of_properties(data, 
    label = r->"Max increase $(Int(r.params[:max_increase_percentage]*100))%",
), "plots/strings/limited_increase/problems_solved_over_number_of_properties.png")

savefig(time_over_solution_size(data, 
    label = r->"Max increase $(Int(r.params[:max_increase_percentage]*100))%",
    ylim = [0,110]
), "plots/strings/limited_increase/time_over_solution_size.png")
