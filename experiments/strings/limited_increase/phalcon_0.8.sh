...
#SBATCH --output=strings_limited_increase_0.8_phalcon_%j.out # Set name of output log. %j is the Slurm jobId
#SBATCH --error=strings_limited_increase_0.8_phalcon_%j.err  # Set name of error log. %j is the Slurm jobId

export JULIA_DEPOT_PATH=$TMPDIR/julia_depot
mkdir -p $JULIA_DEPOT_PATH

srun julia --project=master-thesis -e 'include("experiments/strings/limited_increase/phalcon.jl")' 0.8