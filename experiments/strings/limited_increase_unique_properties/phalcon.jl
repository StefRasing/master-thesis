...
include("../../../src/phalcon_limited_increase_unique_properties.jl")
...
function phalcon_synthesizer(;
    iterator::PhalconLimitedIncreaseUniqueProperties,
    problem::Problem,
)
...
# params = n -> merge(default_params, (unique_properties = n,))
path = "data/strings/limited_increase_unique_properties/true.jld2"
path_base = "data/strings/limited_increase_unique_properties/false.jld2"
# as = [parse(Float64, ARGS[1])]
# as = [0.7, 0.8, 0.9, 1.0]

data = [
    @benchmark PhalconLimitedIncreaseUniqueProperties params=default_params problems=problems[56:56] grammars=grammars args=arguments path=path
    @benchmark PhalconLimitedIncreaseUniqueProperties params=default_params problems=[] grammars=[] args=arguments path=path_base
    # for n in as
]
...
savefig(problems_solved_over_time(data; 
    label = r->r.iterator == PhalconLimitedIncreaseUniqueProperties ? "Unique properties" : "Non unique properties", 
    xscale = :log10,
    xticks = [10.0^n for n in 0:5],
    xlims = [10^0/1.1, 1.1*10^4],
    legend = :topleft,
), "plots/strings/limited_increase_unique_properties/problems_solved_over_time.png")

savefig(problems_solved_over_enumerations(data;
    label = r->r.iterator == PhalconLimitedIncreaseUniqueProperties ? "Unique properties" : "Non unique properties",
    xscale = :log10,
    xticks = [10.0^n for n in 2:8],
    xlims = [10^2/1.1, 1.1*10^8],
    legend = :topleft,
), "plots/strings/limited_increase_unique_properties/problems_solved_over_enumerations.png")

savefig(problems_solved_over_number_of_properties(data, 
    label = r->r.iterator == PhalconLimitedIncreaseUniqueProperties ? "Unique properties" : "Non unique properties",
), "plots/strings/limited_increase_unique_properties/problems_solved_over_number_of_properties.png")

savefig(time_over_solution_size(data, 
    label = r->r.iterator == PhalconLimitedIncreaseUniqueProperties ? "Unique properties" : "Non unique properties",
    ylim = [0,110]
), "plots/strings/limited_increase_unique_properties/time_over_solution_size.png")
