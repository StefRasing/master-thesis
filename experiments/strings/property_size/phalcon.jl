...
params = n -> merge(default_params, (max_property_depth = first(n), max_property_size = last(n),))
path = n -> "data/strings/property_size/d=$(first(n))-s=$(last(n)).jld2"
# property_sizes = [(parse(Int64, ARGS[1]), parse(Int64, ARGS[2]))]
property_sizes = [(4,5),(5,7)]

data = [
    @benchmark Phalcon params=params(n) problems=[] grammars=grammars args=arguments path=path(n) 
    for n in property_sizes
]

savefig(problems_solved_over_time(data; 
    label = r->"Propety depth $(r.params[:max_property_depth]), size $(r.params[:max_property_size])", 
    title = "Problem solved over time for Phalcon",
    xscale = :log10,
    xticks = [10^-2, 10^-1, 10^0, 10^1, 10^2],
    xlims = [10^-1.5, 10^2.5],
    legend = :topleft,
), "plots/strings/property_size/problems_solved_over_time.png")
