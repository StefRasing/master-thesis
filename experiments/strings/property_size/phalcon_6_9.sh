...
#SBATCH --output=strings_poolsize_5_phalcon_%j.out # Set name of output log. %j is the Slurm jobId
#SBATCH --error=strings_poolsize_5_phalcon_%j.err  # Set name of error log. %j is the Slurm jobId

export JULIA_DEPOT_PATH=$TMPDIR/julia_depot
mkdir -p $JULIA_DEPOT_PATH

srun julia --project=master-thesis -e 'include("experiments/strings/property_size/phalcon.jl")' 6 9 